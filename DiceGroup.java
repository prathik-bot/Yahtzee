 ...
 *
 *  @author Mr Greenstein and Prathik Kumar
 *  @since	Oct 6, 2023
*/

public class DiceGroup {
	Dice[] die = new Dice[5]; // holds all form of dice
	...
	public void rollDice(String rawHold) {
		// boolean array to keep track of which dice to roll
		boolean[] rollThese = new boolean[die.length];

		// Initialize all dice to roll by default (true)
		for (int i = 0; i < rollThese.length; i++) {
			rollThese[i] = true;
		}

		// Don't roll dice marked as false
		for (int i = 0; i < rawHold.length(); i++) {
			char c = rawHold.charAt(i);
			if (Character.isDigit(c)) {
				int index = Character.getNumericValue(c) - 1;
				if (index >= 0 && index < rollThese.length) {
					rollThese[index] = false;
				}
			}
		}

		// Roll only the dice that should be rolled
		for (int i = 0; i < die.length; i++) {
			if (rollThese[i]) {
				die[i].roll();
			}
